"""Прогон книги через компилятор, чанками -> KB. argv: <md> <prefix> [lo]."""
...
paras = paragraphs(MD)
LO, HI, CHUNK = (int(sys.argv[3]) if len(sys.argv) > 3 else 0), len(paras), 49
...
metas_path = OUT / f'metas_{PREFIX}.jsonl'
