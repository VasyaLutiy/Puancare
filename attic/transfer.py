"""transfer — ИСТОРИЧЕСКИЙ ДОКУМЕНТ тупика «перенос-как-приор». Не запускать.

Идея была: узнали новый мир как форму F из библиотеки → приорная скидка в
битах структуре с k=F.k → судья покупает онтологию на меньших данных (×6).
Вердикт (SPECTRA §3 + прогон --auto в коммите 7a6000b): ×6 существовал
только при god's-eye подсказке; честная автономная петля ни разу не узнала
родича (подпись недоученного мира ≠ подпись доученного) → засев не включился
→ АВТО ≡ ГОЛЫЙ. Циркулярность: чтобы узнать форму, надо выучить мир.

Seed-хук (_seed_k/_seed_bonus) удалён из ядра — organism2 заморожен как
чистый ученик. Живая замена переносу: fastpath (узнать→подогнать) +
compose (ввоз карты по роли). Воспроизведение тупика: git checkout 7a6000b.
"""
...
if __name__ == "__main__":
    raise SystemExit(
        "transfer.py — исторический документ (перенос-как-приор, тупик).\n"
        "Seed-хук удалён из ядра: засев стал бы тихим no-op, а тихий no-op —\n"
        "это ложь о результате. Воспроизведение: git checkout 7a6000b.")
